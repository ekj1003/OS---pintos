...
thread_tick (void) 
{
  ...
  /*Project #3*/
	// if(thread_prior_aging == true) thread_aging(thread_prior_aging);
               ...
               thread_func *function, void *aux) 
{
  ...
  /*If there is a new thread of which priority is higher than a current thread, 
  CPU should be immediately yielded to the new thread by the current thread. */
...
thread_set_priority (int new_priority) 
{
  ...
  /* Store priority of current thread */
  int old_priority = thread_current()->priority;
  /* Set new priority as priority of current thread */
  thread_current ()->priority = new_priority;

  /* if new priority is lower than old priority of current thread, schedule threads again */
...
void update_priority() {
    ...
    for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
        ...
        priority = PRI_MAX * FRACTION - (t->recent_cpu / 4) - (t->nice * 2) * FRACTION;
        t->priority = priority / FRACTION;
...
void update_recent_cpu() {
    ...
    int64_t load_avg_times_2 = load_avg * 2;
    int64_t coefficient = (load_avg_times_2 * FRACTION) / (load_avg_times_2 + FRACTION);
    ...
    for (e = list_begin(&all_list); e != list_end(&all_list); e = list_next(e)) {
        ...
        if (t != idle_thread) {
            ...
            t->recent_cpu = (coefficient * recent_cpu) / FRACTION + t->nice * FRACTION;
        }
    }
}
...
bool
priority_compare(const struct list_elem *p, const struct list_elem *n, void *aux UNUSED)
{
	struct thread *th1 = list_entry(p, struct thread, elem);
	struct thread *th2 = list_entry(n, struct thread, elem);
