...
struct vm_entry *check_address (void *vaddr, void *esp) {
  ...
  if(!is_user_vaddr(vaddr)) exit(-1);
	...
	if (vmEntry == NULL) {
		if (!verify_stack (vaddr, esp))
			exit (-1);
		expand_stack(vaddr);
		vmEntry = find_vme(vaddr);
	}
	return vmEntry;
...
void check_valid_string (void *str, void *esp) {
	struct vm_entry *vmEntry = check_address (str, esp);
	if (vmEntry == NULL)
		exit (-1);

	int size = 0;
	while (((char *) str)[size] != '\0') size = size + 1;
...
void check_valid_string_size (void *str, unsigned size, void *esp) {
  ...
  while(i<size) {
    struct vm_entry *vmEntry;
    vmEntry = check_address ((void *) (str++), esp);
		if (vmEntry == NULL) exit (-1);
    i++;
	}
}
