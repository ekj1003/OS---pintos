...
process_execute (const char *file_name) 
{
  ...
  /* Copying file_name */
  char* file_copy = (char*)malloc(sizeof(char)*(strlen(file_name) + 1));
  char* saveptr;
  char* cmd;
  
  strlcpy(file_copy, file_name, strlen(file_name) + 1);

  cmd = strtok_r(file_copy, " ", &saveptr);
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  for(e = list_begin(&(thr->child)); e != list_end(&(thr->child)); e = list_next(e))
  {
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Parsing a file_name into a command and variables */
  strlcpy(file_copy, file_name, strlen(file_name) + 1);

  temp=strtok_r(file_copy, delim, &saveptr);

  // Finding argc and Storing variables in argv[1] ~ argv[argc-1]
  while(temp != NULL) {
    parsed[argc] = temp;
          ...
          if (validate_segment (&phdr, file)) 
            {
              ...
              if (!load_segment (parsed[0], file, file_page, (void *) mem_page,
 ...
 /* Making a stack (Argument Passing) */
  *esp = PHYS_BASE;

  // argv: pointer saving addresses
  int tlen = 0;
  int len;
  ...
  **(uint32_t **)esp = *esp + 4;
  ...
  // Stacking the number of arguments
  *esp -= 4;
  **(uint32_t **)esp = argc;
  
  // Stacking return address
  *esp -= 4;
  **(uint32_t**)esp = 0;
              ...
              uint32_t read_bytes, uint32_t zero_bytes, bool writable) 
{
  ...
  while (read_bytes > 0 || zero_bytes > 0) {
    ...
    struct vm_entry *vme = malloc (sizeof (struct vm_entry));
		vme->file_name = (char *) malloc (sizeof (char) * strlen(file_name));
		strlcpy(vme->file_name, file_name, strlen(file_name) + 1);
		vme->type = VM_BIN;
		vme->vaddr = upage;
...
bool expand_stack (void *addr) {
  ...
  kpage = alloc_page (PAL_USER | PAL_ZERO);
  vmEntry = malloc (sizeof (struct vm_entry));

  
	if (vmEntry==NULL) ans = false;
...
bool verify_stack (void *fault_addr, void *esp) {
  ...
  bool ans = is_user_vaddr (pg_round_down (fault_addr)) && fault_addr >= esp - 32 && fault_addr >= limit;
