          ...
          if (validate_segment (&phdr, file)) 
            {
              ...
              if (!load_segment (file, file_page, (void *) mem_page,
                                 read_bytes, zero_bytes, writable, parsed[0]))
                goto done;
            }
          else
            goto done;
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      size_t page_zero_bytes = PGSIZE - page_read_bytes;
		...
		vme->file = (char *) malloc (sizeof (char) * strlen(filename));
		strlcpy(vme->file, filename, strlen(filename) + 1);
...
bool expand_stack (void *addr) {
	...
	if (install_page (vme->vaddr, kpage->kaddr, vme->writable)) return true;
	else {
		free_page (kpage->kaddr);
		free (vme);
		return false;
...
bool verify_stack (void *fault_addr, void *esp) {	
	...
	void *limit;
	bool ans;
	limit = PHYS_BASE - 8 * 1024 * 1024;
	ans = is_user_vaddr (pg_round_down (fault_addr)) && fault_addr >= esp - 32 && fault_addr >= limit;
	return ans;
}
